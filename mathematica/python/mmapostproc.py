...
def pop_stack (index, stack_index):
   if stack_index > 0:
      if index == read_stack (stack_index):
         ...
         print ("> Error with mmaBeg/mmaEnd pairs for tag index: "+str(index)+", exit\n")
         sys.exit (1)
   else:
...
def tex_macro (tex, index):
   ...
   tex.write ("\mmatag{"+tag_name[index]+"}{%\n")
...
parser = argparse.ArgumentParser(description="Post-process Mathematica output")
parser.add_argument("-i", dest="input", metavar="source", help="LaTeX-Mathematica source file (without .tex file extension)", required=True)
parser.add_argument("-I", dest="sty", metavar="include", help="Full path to LaTeX-Mathematica mmamacros.sty file")

the_file_name = parser.parse_args().input
sty_file_name = parser.parse_args().sty

# ----------------------------------------------------------------------------
# include the Mathematica macros in the .cdbtex file?

if sty_file_name:
    include_macros_header = True
    if not os.path.isfile (sty_file_name):
       print ("> could not find "+sty_file_name)
       print ("> will not include Mathematica macros")
...
idx_file_name = the_file_name + ".mmaidx"
tex_file_name = the_file_name + ".mmatex"
src_file_name = the_file_name + ".mmatxt"
...
if not os.path.isfile (idx_file_name):
   with open(tex_file_name,"w") as tex:
      tex.write ("% no Mathematica output")
   sys.exit (0)
...
# note: num_tag = number of tags declared in the Mathematica/LaTeX source
#       these tags are stored in locations 1,2,3 ... num_tag in the various arrays
#       tag_output[0] will contain all non-tagged Mathematica output.

# ----------------------------------------------------------------------------
# read Mathematica output and create LaTeX macros for each tag
...
if num_tag == 0:
   ...
   if not os.path.isfile (src_file_name):
      ...
      print ("> possible error during execution of Mathematica.")
      sys.exit (1)

   # -------------------------------------------------------------------------
   # read Mathematica output
   ...
   with open(tex_file_name,"w") as tex:
      ...
      if include_macros_header:
         tex.write(r'% ====================================================================='+'\n')
         tex.write(r'% Define Mathematica macros so that this file may be used by other LaTeX sources.'+'\n')
         ...
         tex.write(r'% \input{foo.mmatex}% change foo to match the name of this file.'+'\n')
         tex.write(r'% ---------------------------------------------------------------------'+'\n')
   ...
   # -------------------------------------------------------------------------
   # report tags that didn't have matching Mathematica output
